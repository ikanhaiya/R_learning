...
# vector also can be created using the vector keyword 

x <- vector

# when two vectors are mixed then vectors are coerced meaning numbers are converted to characters

x = c(1:10) # takes vector of values from 1 to 10 
x
y = c(1:10, 15:20) # gives vector of values from 1 to 10 then 15 to 20
y
...
y <- c(1.7, 'a') # character type 
typeof(y)
class(y)

z <- c(TRUE, 2) # numeric type.
typeof(z)
class(z)

y <- c(1.2, 'a')  # character type 
class(y)

x <- c('a', 'b', 'c')
as.numeric(x) # gives Warning message: NAs introduced by coercion
as.logical(x) # [1] NA NA NA
as.complex(x) # gives:  Warning message: NAs introduced by coercion


# Attributes 
# R object have attributes (metadata for object)
# eg of R object attributes (names, dimnames, dimensions(matrices, arrays), class( numeric, characteetc)
# , length and other user defined attributes)

# Not all objects contains attributes in that case attribute function returns null
